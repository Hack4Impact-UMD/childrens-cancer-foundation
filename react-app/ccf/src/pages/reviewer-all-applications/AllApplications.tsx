...
function AllApplications(): JSX.Element {
  const [applicationsData, setApplicationsData] = useState<{
    [year: string]: { coverSheets: Application[] };
  }>({});
  const [collapseState, setCollapseState] = useState<{
    [year: string]: { coverSheets: boolean; isYearCollapsed: boolean };
  }>({});
  ...
  useEffect(() => {
        ...
        coverSheets: [
          {
            ...
            applicationType: "Application Type (Next Gen/Research)",
            decision: "",
            cancerType: "",
          },
          {
    ...
    const initialCollapseState = Object.keys(initialData).reduce(
      (acc, year) => {
  ...
  const filteredApplications = Object.keys(applicationsData).reduce(
    (acc, year) => {
      const filteredYearData = {
        coverSheets: applicationsData[year].coverSheets.filter(
          (app) =>
            ...
            (filters.grantType
              ? app.applicationType.includes(filters.grantType)
              : true) &&
            (filters.institution
                  ...
                  {!collapseState[year]?.isYearCollapsed && (
                    <>
                      <div className="applications-container">
                        {!collapseState[year]?.coverSheets &&
                          applicationsData[year].coverSheets.map(
                            (coverSheet, index) => (
                              <div
                                key={index}
                                className="single-application-box"
                              >
